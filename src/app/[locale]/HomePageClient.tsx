...
const EXPIRY_OPTIONS = [
  { value: 1, labelKey: '1day', minPlan: 'free' },
  { value: 3, labelKey: '3days', minPlan: 'free' },
  { value: 5, labelKey: '5days', minPlan: 'free' },
  { value: 7, labelKey: '1week', minPlan: 'starter' },
  { value: 14, labelKey: '2weeks', minPlan: 'pro' },
  { value: 30, labelKey: '1month', minPlan: 'pro' },
  { value: 90, labelKey: '3months', minPlan: 'business' },
  { value: 365, labelKey: '1year', minPlan: 'business' },
];

const PLAN_HIERARCHY = ['free', 'starter', 'pro', 'business'];

// Access control options (labels/descriptions are translation keys under home.upload.accessControl)
...
// View options (labels/descriptions are translation keys under home.upload.viewOptions)
const VIEW_OPTIONS = [
  { value: 'preview_download' as const, labelKey: 'previewDownload', descKey: 'previewDownloadDesc' },
  { value: 'download_only' as const, labelKey: 'downloadOnly', descKey: 'downloadOnlyDesc' },
  ...
  if (subView === 'access') {
    return (
      <div>
        ...
        <h3 className="text-lg font-bold text-gray-900 mb-5">{t('upload.accessControlTitle')}</h3>
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <span className={`text-sm font-semibold ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>
                        ...
                        {t(`upload.accessControl.${opt.labelKey}`)}
                      </span>
                    </div>
                    <p className="text-xs text-gray-500 mt-1 leading-relaxed">{t(`upload.accessControl.${opt.descKey}`)}</p>
                  </div>
                </div>
              </button>
            );
    ...
    <div className="space-y-0">
        ...
        >
          <div className="text-left">
            ...
            <p className={`text-sm font-medium mt-0.5 ${isBusiness ? 'text-gray-900' : 'text-gray-400'}`}>
              {t(`upload.accessControl.${ACCESS_OPTIONS.find(o => o.value === accessControl)?.labelKey || 'public'}`)}
            </p>
          </div>
          {isBusiness && (
      ...
      <div className="border-b border-gray-100 pb-4 mb-4 relative">
        ...
        >
          <div className="text-left">
            ...
            <p className={`text-sm font-medium mt-0.5 ${isBusiness ? 'text-gray-900' : 'text-gray-400'}`}>
              {t(`upload.viewOptions.${VIEW_OPTIONS.find(o => o.value === viewOption)?.labelKey || 'previewDownload'}`)}
            </p>
          </div>
          {isBusiness && (
          ...
          <div className="lg:hidden mt-3 space-y-0 border border-gray-100 rounded-xl overflow-hidden">
            {VIEW_OPTIONS.map((opt, idx) => {
              ...
              return (
                ...
                >
                  <span className={`text-sm font-semibold ${isSelected ? 'text-gray-900' : 'text-gray-700'}`}>
                    {t(`upload.viewOptions.${opt.labelKey}`)}
                  </span>
                  <p className="text-xs text-gray-500 mt-0.5 leading-relaxed">{t(`upload.viewOptions.${opt.descKey}`)}</p>
                </button>
              );
            })}
          </div>
      ...
      <div className="border-b border-gray-100 pb-4 mb-4">
        ...
        <input
          ...
          placeholder={canUsePassword ? t('upload.setPassword') : t('upload.proPlanRequired')}
        />
      </div>
...
export default function HomePageClient() {
  ...
  const sendVerificationCode = async () => {
    if (!senderEmail.trim()) {
      setUploadError(t('upload.enterEmailToProceed'));
      return false;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        if (result.limitsExceeded) {
          ...
          setVerificationError(result.error || t('upload.codeSendError'));
        }
        return false;
      }
    } catch (error) {
      console.error('Error sending verification code:', error);
      setUploadError(t('upload.connectionError'));
      return false;
    } finally {
  ...
  const verifyCode = async (codeToVerify?: string) => {
    ...
    if (!code || code.length !== 6) {
      setVerificationError(t('verification.enterSixDigits'));
      return;
    }
    ...
    try {
      ...
      if (result.success) {
        ...
        setVerificationError(result.error || t('verification.invalidCode'));
      }
    } catch (error) {
      console.error('Error verifying code:', error);
      setVerificationError(t('upload.connectionError'));
    } finally {
  ...
  const performUpload = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : t('upload.uploadError');
  ...
  const handleUpload = async () => {
    ...
    if (!title.trim()) {
      setUploadError(t('upload.enterTitleError'));
      return;
    }
    ...
    if (deliveryMethod === 'email' && !recipientEmail.trim()) {
      setUploadError(t('upload.enterRecipientEmail'));
      return;
    }
  ...
  return (
    <MainLayout showFooter={false} transparentBg>
      ...
      <div className={`${showAdvancedOptions ? 'min-h-dvh overflow-y-auto' : 'h-dvh overflow-hidden'} lg:h-auto lg:min-h-screen lg:overflow-x-hidden lg:overflow-y-visible -mt-16 pt-16 relative bg-blue-500 lg:bg-gradient-to-br lg:from-blue-500 lg:via-purple-500 lg:to-pink-500`}>
        {/* Decorative circles — hidden on mobile for solid color */}
        <div className="hidden lg:block absolute top-20 left-10 w-72 h-72 bg-white/10 rounded-full blur-3xl" />
        <div className="hidden lg:block absolute bottom-10 right-20 w-96 h-96 bg-purple-300/20 rounded-full blur-3xl" />
                ...
                <div className="mb-4">
                  <div className="flex justify-between items-center mb-2">
                    <span className="text-sm font-semibold text-gray-700">
                      {t('upload.fileCount', { count: files.length, size: formatBytes(totalSize) })}
                    </span>
                    <button
                      type="button"
                      onClick={() => document.getElementById('fileInput')?.click()}
              ...
              {files.length === 0 && (
                <p className="text-xs text-gray-400 mb-4">
                  {planLimits.maxFilesPerTransfer === -1
                    ? t('upload.maxFilesUnlimited')
                    : t('upload.maxFiles', { max: planLimits.maxFilesPerTransfer })}
                </p>
              )}
                  ...
                  >
                    {availableExpiryOptions.map(option => (
                      <option key={option.value} value={option.value}>
                        {t(`upload.expiryOptions.${option.labelKey}`)}
                      </option>
                    ))}
                  </select>
                </div>
                  ...
                  <>
                    ...
                    {files.length > 0
                      ? isAnonymous && !isEmailVerified
                        ? t('upload.transfer')
                        : t('upload.transferWithSize', { size: formatBytes(totalSize) })
                      : t('upload.transfer')
                    }
                  </>
                )}
                  ...
                  {VIEW_OPTIONS.map((opt, idx) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <p className="text-xs text-gray-500 mt-1 leading-relaxed">{t(`upload.viewOptions.${opt.descKey}`)}</p>
                      </button>
                    );
                  })}
                </div>
