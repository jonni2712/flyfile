...
export default function PricingContent() {
  ...
  // Build plan display data from translations
  const planIds = ['free', 'starter', 'pro', 'business'] as const;
  const translatedPlans = PRICING_PLANS.map((plan) => {
    const pid = plan.id as typeof planIds[number];
    const featureCount = plan.features.length;
    const features: string[] = [];
    for (let i = 0; i < featureCount; i++) {
      features.push(tp(`plans.${pid}.features.${i}`));
    }
    return {
      ...plan,
      name: tp(`plans.${pid}.name`),
      description: tp(`plans.${pid}.description`),
      cta: tp(`plans.${pid}.cta`),
      badge: pid === 'business' ? tp('plans.business.badge') : plan.badge,
      features,
    };
  });

  // Build comparison sections from translations
          ...
          {translatedPlans.map((plan, idx) => {
            ...
            return (
              ...
              >
                ...
                {/* CTA */}
                <button
                  onClick={() => !isCurrentPlan && handleSubscribe(PRICING_PLANS[idx])}
                ...
                >
                  {isCurrentPlan ? tp('activePlan') : checkoutLoading === plan.id ? tp('loading') : plan.cta}
                </button>
              </div>
            );
          })}
