...
export async function GET(request: NextRequest) {
  try {
    ...
    // Process expired transfers in batches until all are handled or max limit reached
    while (hasMore && deletedCount < MAX_TOTAL) {
      ...
      // Process each expired transfer in this batch
      for (const transferDoc of expiredTransfersSnapshot.docs) {
        if (deletedCount >= MAX_TOTAL) {
          break;
        }
      ...
      // If this batch was smaller than BATCH_SIZE, no more expired transfers
      if (expiredTransfersSnapshot.size < BATCH_SIZE) {
        hasMore = false;
      }
    }
    ...
    return NextResponse.json({
      ...
      deletedCount,
      totalSizeFreedMB,
      hasMore: hasMore && deletedCount >= MAX_TOTAL ? 'More expired transfers pending' : 'All processed',
